...
<template>
  ...
  <DropdownMenu v-if="user">
    ...
    <DropdownMenuContent align="end" class="w-56 bg-background/95 backdrop-blur-xl border-white/10">
      <DropdownMenuLabel class="font-normal">
        <div class="flex flex-col space-y-1">
          <p class="text-sm font-medium leading-none">{{ user.user_metadata?.full_name || 'User' }}</p>
          <p class="text-xs leading-none text-muted-foreground">{{ user.email }}</p>
        </div>
      </DropdownMenuLabel>
      <DropdownMenuSeparator class="bg-white/10" />
      <DropdownMenuItem @click="router.push('/account')" class="cursor-pointer focus:bg-white/10">
        <User class="mr-2 h-4 w-4" />
        Dashboard
      </DropdownMenuItem>
      <DropdownMenuItem @click="router.push('/account/orders')" class="cursor-pointer focus:bg-white/10">
        <Package class="mr-2 h-4 w-4" />
        My Orders
      </DropdownMenuItem>
      <DropdownMenuItem @click="router.push('/account/settings')" class="cursor-pointer focus:bg-white/10">
        <Settings class="mr-2 h-4 w-4" />
        Settings
      </DropdownMenuItem>
      <DropdownMenuSeparator class="bg-white/10" />
      <DropdownMenuItem @click="handleLogout" class="cursor-pointer focus:bg-white/10 text-destructive focus:text-destructive">
